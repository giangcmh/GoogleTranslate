...
caradditionalservicedetails = pd.read_sql("select	* from	[dbo].[CarAdditionalServiceDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[CarAdditionalServiceDetails] B where A.AdditionalServiceFID = B.AdditionalServiceFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# CarCancelPoliMutilLang
carcancelpolimutillang = pd.read_sql("select	* from	[dbo].[CarCancelPoliMutilLang] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[CarCancelPoliMutilLang] B where A.CancellationPoliciFID = B.CancellationPoliciFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# CarInformationDetails
carinformationdetails = pd.read_sql("select	* from	[dbo].[CarInformationDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[CarInformationDetails] B where A.InformationFID = B.InformationFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# CarPaymentPoliMutilLang
carpaymentpolimutillang = pd.read_sql("select	* from	[dbo].[CarPaymentPoliMutilLang] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[CarPaymentPoliMutilLang] B where A.PaymentPoliciFID = B.PaymentPoliciFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')
...
# CarAdditionalServiceDetails
for index, row in caradditionalservicedetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[CarAdditionalServiceDetails]([AdditionalServiceFID],[LanguageFID],[Name],[Remark],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,getdate(),?,NULL)",
...
# CarCancelPoliMutilLang
for index, row in carcancelpolimutillang.iterrows():
    cursor.execute("INSERT INTO [dbo].[CarCancelPoliMutilLang]([CancellationPoliciFID],[LanguageFID],[Name],[TermAndPolicies],[IsActive],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL)",
...
# CarInformationDetails
for index, row in carinformationdetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[CarInformationDetails]([UniqueID],[InformationFID],[LanguageFID],[FileTypeFID],[FileStreamFID],[Title],[ShortDescriptions],[FullDescriptions],[Deleted],[IsActivated],[ActivatedDate],[ActivatedBy],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,?,?,?,getdate(),?,?,NULL)",
                   row.UniqueID,
                   row.InformationFID,
                   languagefid,
                   row.FileTypeFID,
...
# CarPaymentPoliMutilLang
for index, row in carpaymentpolimutillang.iterrows():
    cursor.execute("INSERT INTO [dbo].[CarPaymentPoliMutilLang]([PaymentPoliciFID],[LanguageFID],[Name],[TermAndPolicies],[IsActive],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL)",
