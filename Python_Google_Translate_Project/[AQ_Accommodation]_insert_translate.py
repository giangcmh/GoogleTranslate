...
hotelinformationdetails = pd.read_sql("select * from [dbo].[HotelInformationDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[HotelInformationDetails] B where A.InformationFID = B.InformationFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')
...
roomamenityattributedetails = pd.read_sql("select * from [dbo].[RoomAmenityAttributeDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[RoomAmenityAttributeDetails] B where A.RoomAmenitiFID = B.RoomAmenitiFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# HotelSandboxMutilLang
hotelsandboxmutillang = pd.read_sql("select * from [dbo].[HotelSandboxMutilLang] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[HotelSandboxMutilLang] B where A.SanboxFID = B.SanboxFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# HotelAmenityAttributeDetails
hotelamenityattributedetails = pd.read_sql("select * from [dbo].[HotelAmenityAttributeDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[HotelAmenityAttributeDetails] B where A.HotelAmenitiFID = B.HotelAmenitiFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# HotelAdditionalServiceDetails
hoteladditionalservicedetails = pd.read_sql("select * from [dbo].[HotelAdditionalServiceDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[HotelAdditionalServiceDetails] B where A.AdditionalServiceFID = B.AdditionalServiceFID and B.LanguageFID = 5)  and Deleted = 0"
                   , cnxn, index_col='ID')
...
for index, row in hotelinformationdetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[HotelInformationDetails]([UniqueID],[InformationFID],[LanguageFID],[FileTypeFID],[FileStreamFID],[Title],[ShortDescriptions],[FullDescriptions],[Deleted],[IsActivated],[ActivatedDate],[ActivatedBy],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,?,?,?,getdate(),?,?,NULL)",
                   row.UniqueID,
                   row.InformationFID,
                   languagefid,
                   row.FileTypeFID,
...
# RoomAmenityAttributeDetails
for index, row in roomamenityattributedetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[RoomAmenityAttributeDetails]([RoomAmenitiFID],[LanguageFID],[Name],[Remark],[IsActive],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL)",
                   row.RoomAmenitiFID,
                   languagefid,
                   row.Description_Translated,
                   row.Remark,
...
# HotelSandboxMutilLang
for index, row in hotelsandboxmutillang.iterrows():
    cursor.execute("INSERT INTO [dbo].[HotelSandboxMutilLang]([SanboxFID],[LanguageFID],[SanboxName],[SanboxDescription],[IsActivated],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate],[SandboxPolicies],[SanboxShortDescription]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL,?,?)",
                   row.SanboxFID,
                   languagefid,
                   row.SanboxName,
                   row.Description_Translated,
...
# HotelAmenityAttributeDetails
for index, row in hotelamenityattributedetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[HotelAmenityAttributeDetails]([HotelAmenitiFID],[LanguageFID],[Name],[Remark],[IsActive],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL)",
                   row.HotelAmenitiFID,
                   languagefid,
                   row.Description_Translated,
                   row.Remark,
...
# HotelAdditionalServiceDetails
for index, row in hoteladditionalservicedetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[HotelAdditionalServiceDetails]([AdditionalServiceFID],[LanguageFID],[Name],[Remark],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,getdate(),?,NULL)",
