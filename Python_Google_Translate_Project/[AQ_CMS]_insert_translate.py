...
# PostCategoryDetails
postcategorydetails = pd.read_sql("select	* from	[dbo].[PostCategoryDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[PostCategoryDetails] B where A.PostCategoryFID = B.PostCategoryFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# PostDetails
postdetails = pd.read_sql("select	* from	[dbo].[PostDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[PostDetails] B where A.PostFId = B.PostFId and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')
...
# postcategorydetails
for index, row in postcategorydetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[PostCategoryDetails]([PostCategoryFID],[Name],[LanguageFID],[Deleted],[IsActivated],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate],[FriendlyUrl]) VALUES (?,?,?,?,?,?,getdate(),?,NULL,?)",
                   row.PostCategoryFID,
...
# postdetails
for index, row in postdetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[PostDetails]([UniqueId],[PostFId],[LanguageFID],[FileTypeFID],[FileStreamFID],[Title],[MetaDescription],[Body],[KeyWord],[Deleted],[IsActivated],[ActivatedDate],[ActivatedBy],[LastModifiedBy],[LastModifiedDate],[ShortDescription],[FriendlyUrl]) VALUES (?,?,?,?,?,?,?,?,?,?,?,getdate(),?,?,NULL,?,?)",
                   row.UniqueId,
                   row.PostFId,
                   languagefid,
                   row.FileTypeFID,
