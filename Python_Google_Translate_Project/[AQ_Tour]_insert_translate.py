...
tourcancelpolimutillang = pd.read_sql("select	* from	[dbo].[TourCancelPoliMutilLang] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[TourCancelPoliMutilLang] B where A.CancellationPoliciFID = B.CancellationPoliciFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# TourInformationDetails
tourinformationdetails = pd.read_sql("select	* from	[dbo].[TourInformationDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[TourInformationDetails] B where A.InformationFID = B.InformationFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# TourPaymentPoliMutilLang
tourpaymentpolimutillang = pd.read_sql("select	* from	[dbo].[TourPaymentPoliMutilLang] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[TourPaymentPoliMutilLang] B where A.PaymentPoliciFID = B.PaymentPoliciFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# TourPricingDetails
tourpricingdetails = pd.read_sql("select	* from	[dbo].[TourPricingDetails] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[TourPricingDetails] B where A.PricingFID = B.PricingFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# TourTripPlanDetail
tourtripplandetail = pd.read_sql("select	* from	[dbo].[TourTripPlanDetail] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[TourTripPlanDetail] B where A.TripPlanFID = B.TripPlanFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')

# TourTypeDetail
tourtypedetail = pd.read_sql("select	* from	[dbo].[TourTypeDetail] A where	LanguageFID = 1 and not exists (select 1 from [dbo].[TourTypeDetail] B where A.TourTypeFID = B.TourTypeFID and B.LanguageFID = 5) and Deleted = 0"
                   , cnxn, index_col='ID')
...
# TourCancelPoliMutilLang
for index, row in tourcancelpolimutillang.iterrows():
    cursor.execute("INSERT INTO [dbo].[TourCancelPoliMutilLang]([CancellationPoliciFID],[LanguageFID],[Name],[TermAndPolicies],[IsActive],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL)",
...
# TourInformationDetails
for index, row in tourinformationdetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[TourInformationDetails]([UniqueID],[InformationFID],[LanguageFID],[FileTypeFID],[FileStreamFID],[Title],[ShortDescriptions],[FullDescriptions],[Deleted],[IsActivated],[ActivatedDate],[ActivatedBy],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,?,?,?,getdate(),?,?,NULL)",
                   row.UniqueID,
                   row.InformationFID,
                   languagefid,
                   row.FileTypeFID,
...
# TourPaymentPoliMutilLang
for index, row in tourpaymentpolimutillang.iterrows():
    cursor.execute("INSERT INTO [dbo].[TourPaymentPoliMutilLang]([PaymentPoliciFID],[LanguageFID],[Name],[TermAndPolicies],[IsActive],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL)",
...
# TourPricingDetails
for index, row in tourpricingdetails.iterrows():
    cursor.execute("INSERT INTO [dbo].[TourPricingDetails]([PricingFID],[LanguageFID],[Name],[Remark],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,getdate(),?,NULL)",
                   row.PricingFID,
                   languagefid,
                   row.Name_Translated,
...
# TourTripPlanDetail
for index, row in tourtripplandetail.iterrows():
    cursor.execute("INSERT INTO [dbo].[TourTripPlanDetail]([TripPlanFID],[ItemName],[ItemDescription],[LanguageFID],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate],[IsActivated]) VALUES (?,?,?,?,?,?,getdate(),?,NULL,?)",
                   row.TripPlanFID,
...
# TourTypeDetail
for index, row in tourtypedetail.iterrows():
    cursor.execute("INSERT INTO [dbo].[TourTypeDetail]([TourTypeFID],[LanguageFID],[Name],[Remark],[IsActive],[Deleted],[CreatedBy],[CreatedDate],[LastModifiedBy],[LastModifiedDate]) VALUES (?,?,?,?,?,?,?,getdate(),?,NULL)",
                   row.TourTypeFID,
                   languagefid,
                   row.Name_Translated,
                   row.Remark,
